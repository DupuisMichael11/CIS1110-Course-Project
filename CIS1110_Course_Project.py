...
print(f"{bardName} loved to take walks through {forestName} forest and play their {instrument} for all the woodland creatures they saw.  ")
print(f"{bardName} would even sing and dance with the woodland creatures from time to time.  ")
print(f"Oneday {bardName} was walking through the forest when he heard the most beautiful sound.  ")
print(f"There were {numberOfBirds} birds flitting through the trees singing merrily")
print(f"{bardName} pulled out their {instrument} and started playing a jaunty tune. ")

#Decision 1
singAlong = input(f"\nShould {bardName} sing along too? Type yes or no:  ")
if singAlong.lower() == "yes":
    print(f"\nWhile {bardName} played along with the birds they got excited and chirped even louder.   ")
    print(f"Then {bardName} decided to sing along while quite out of tune. ")
    print(f"Birds, birds with wings so grand,  ")
    print(f"Let's sing and dance across the land.  ")
    print(f"The sky so wide, you fly so high.  ")
    print(f"Will you come land, upon my hand.  ")
    print(f"\nAs soon as the bard finished their out of tune song, the birds obliged and flew down to land upon {bardName}'s hand...")
    print(f"Then the birds started attacking {bardName}!!!")
    print(f"{bardName} ran and ran deep into {forestName} forest, all scratched and bruised, until they got away.")
else:
